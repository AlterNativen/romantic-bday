...
function timeElapse(date) {
    var current = new Date(); // Use new Date() to get the current date and time as an object
    var seconds = Math.floor((current - Date.parse(date)) / 1000); // Calculate the difference in seconds

    var days = Math.floor(seconds / (3600 * 24)); // Convert seconds to days
    seconds = seconds % (3600 * 24); // Get remaining seconds after calculating days

    var hours = Math.floor(seconds / 3600); // Convert remaining seconds to hours
    seconds = seconds % 3600; // Get remaining seconds after calculating hours
